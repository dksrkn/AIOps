...
def train_model(
...
) -> Tuple[Artifacts, Dict, pd.DataFrame]:
    ...
    if status_dict is not None:
        status_dict["stage"] = "building_sequences"
        status_dict["progress_pct"] = 10.0
    ...
    if status_dict is not None:
        status_dict["stage"] = "training"
        status_dict["progress_pct"] = 15.0
        status_dict["total_epochs"] = EPOCHS
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if status_dict is not None:
            # 15%~90%를 epoch 진행률로 사용
            progress = 15.0 + (epoch / EPOCHS) * 75.0
            status_dict["current_epoch"] = epoch
            status_dict["progress_pct"] = round(progress, 2)
            status_dict["stage"] = "training"
    ...
    if status_dict is not None:
        status_dict["stage"] = "evaluation_done"
...
def load_artifacts() -> Artifacts:
    if not (
        MODEL_STATE_PATH.exists()
        and FEATURE_SCALER_PATH.exists()
        and TARGET_SCALER_PATH.exists()
        and METADATA_PATH.exists()
    ):
        raise FileNotFoundError("모델 아티팩트가 없습니다. 먼저 POST /retrain 으로 학습을 수행하세요.")
...
def prepare_train_history(train_df: pd.DataFrame, building_df: pd.DataFrame, artifacts: Artifacts) -> pd.DataFrame:
    ...
    train_raw["lag1_kwh"] = train_raw["lag1_kwh"].fillna(
        train_raw.groupby("건물번호")[TARGET_COL].transform("median")
    )
    return train_raw
...
def recursive_forecast(artifacts: Artifacts, train_hist_df: pd.DataFrame, future_df: pd.DataFrame) -> np.ndarray:
    ...
    for bno in sorted(future_df["건물번호"].unique()):
        ...
        for _, row in future.iterrows():
            ...
            current_scaled = feature_scaler.transform(
                pd.DataFrame([current_features], columns=scale_feature_cols)
            )[0]
...
def evaluate_predictions(df_with_preds: pd.DataFrame) -> Dict:
    if TARGET_COL not in df_with_preds.columns:
        return {
            "can_evaluate": False,
            "rmse": None,
            "message": f"{TARGET_COL} 컬럼이 없어 RMSE를 계산할 수 없습니다.",
        }
    ...
    if eval_df.empty:
        return {
            "can_evaluate": False,
            "rmse": None,
            "message": "실제 타깃 값이 비어 있어 RMSE를 계산할 수 없습니다.",
        }
...
def retrain_pipeline(
    current_artifacts: Artifacts | None = None,
    status_dict: dict | None = None,
) -> Dict:
    if status_dict is not None:
        status_dict["stage"] = "loading_data"
        status_dict["progress_pct"] = 1.0

    train_df = pd.read_csv(TRAIN_PATH)
    building_df = pd.read_csv(BUILDING_PATH)

    new_artifacts, history, valid_result = train_model(train_df, building_df, status_dict=status_dict)
    new_rmse = new_artifacts.baseline_rmse

    previous_rmse = None if current_artifacts is None else current_artifacts.baseline_rmse
    previous_version = None if current_artifacts is None else current_artifacts.version

    promoted = bool(previous_rmse is None or np.isnan(previous_rmse) or new_rmse < previous_rmse)
    status = "retrained_promoted" if promoted else "retrained_rejected"

    if status_dict is not None:
        status_dict["stage"] = "saving"
        status_dict["progress_pct"] = 97.0
    ...
    log_metrics(
        ...
        True,
        {
            "previous_baseline_rmse": previous_rmse,
